...
//		printf("%d - Beginning creating task %d.\n", generator_id, num_dd_tasks_generated + 1);
		...
		if (num_dd_tasks_generated < num_tasks_per_hyperperiod) {
...
//			xTaskCreate( User_Defined_Task, "User_Defined_Task", configMINIMAL_STACK_SIZE, (void *) &new_dd_task, USER_TASK_PRIORITY, &user_task_handle);
...
static void Complete_Callback(TimerHandle_t timer) {
	...
	Scheduler_Event new_complete_event = {COMPLETE, NULL, &timer};
...
//	printf("Done User_Task - Task ID of: %d - P: %d\n", dd_task->task_id, dd_task->parent_id);
//	dd_task = NULL;
//	vTaskDelete(NULL);

	// We can't actually return or end this task without using vTaskDelete(); but unless we introduce a queue, we can never actually know that... Here, at least.
	while(1);
}
...
static void Scheduler_Task( void *pvParameters )
{
	...
	// This keeps track of events for logging purposes.
